...
UNSIGNED_INTEGER_NAMES: List[str] = [
    "uint",
    "ue",
    "uie",
    "uintbe",
...
SHORT_TOKEN_RE: Pattern[str] = re.compile(
    r"^(?P<name>[uifboh]):?(?P<len>\d+)?(=(?P<value>.*))?$"
)
DEFAULT_BITS: Pattern[str] = re.compile(
...
FIXED_LENGTH_TOKENS: Dict[str, int] = {
    "bool": 1,
    "bfloat": 16,
    "float8_143": 8,
    "float8_152": 8,
...
def parse_name_length_token(fmt: str) -> Tuple[str, int]:
    ...
    if name in FIXED_LENGTH_TOKENS.keys():
        ...
        if length not in [0, token_length]:
            raise ValueError(
                f"{name} tokens can only be {token_length} bits long, not {length} bits."
            )
        length = token_length
            ...
            if length is not None:
                ...
                try:
                    ...
                    if not keys or length not in keys:
                        raise ValueError(
                            f"Don't understand length '{length}' of token."
                        )
