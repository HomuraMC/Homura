...
lut_float16_to_float8_143_compressed = (
    b"x\x01\xed\xdde\xb6\x96\x05\x00E\xe1\x8f\xee\x06\xe9FZA\xa4\xbb\xbb;\xa4SB\xba\xeb\xd2\xdd\x8dt\x97\x92J(\xa14\xa2\x84"
...
lut_float16_to_float8_152_compressed = (
    b'x\x01\xed\xdde\xa2\x16T\x00E\xd1\x07\n\x82R\x92J\nJHK7"\xa1H#\xd2HIwwI\xa7R\xd2H\xab(\xdd%(\xadt\x89A\xa7\xd2\xdd1\x8c'
...
lut_int8_to_float8_143_compressed = (
    b'x\x01\x15\xcc[\xb5\x90!\x10\x80Q"\x18\x81\x08<\xabGQ\x0b\x10\x81\x084\x90\x08D \x02\xcf^\xd1S\xe0\x8f@\x04"\xb8e\xad=/3\x1f!\xfc\x7f\xfd\xad\xf1.\x84Lg\xb29'
...
lut_int8_to_float8_152_compressed = (
...
class FP8Format:
    ...
    def __init__(self, exp_bits: int, bias: int):
        ...
        if exp_bits == 4 and bias == 8:
            self.lut_float16_to_float8 = zlib.decompress(
                lut_float16_to_float8_143_compressed
            )
            self.lut_int8_to_float = struct.unpack(
                "<256f", zlib.decompress(lut_int8_to_float8_143_compressed)
            )
            ...
            self.lut_int8_to_float = struct.unpack(
                "<256f", zlib.decompress(lut_int8_to_float8_152_compressed)
            )
        else:  # pragma: no cover
            raise RuntimeError(
                "Unsupported float8 format trying to be created. Only float8_143 and float8_152 available for now."
            )
