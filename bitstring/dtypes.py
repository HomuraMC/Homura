...
import functools
from bitstring.utils import (
    parse_name_length_token,
...
class Dtype:
    ...
    def __init__(self, fmt: str) -> None:
        ...
        try:
            self.get = functools.partial(
                Bits._name_to_read[self.name], length=self.length
            )
        except KeyError:
            raise ValueError(f"The token '{self.name}' is not a valid Dtype name.")
    ...
    def __str__(self) -> str:
        length_str = (
            "" if (self.length == 0 or self.is_fixedlength) else str(self.length)
        )
