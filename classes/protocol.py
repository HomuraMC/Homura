...
class HomuraServerProtocol(ServerProtocol):
    ...
    builtins.emptyHeight = TagRoot(
        {"": TagCompound({"MOTION_BLOCKING": TagLongArray(PackedArray.empty_height())})}
    )

    builtins.registry = LookupRegistry.from_jar(
        os.path.join(os.getcwd(), "assets", "registry", "server.jar")
    )
    ...
    def player_joined(self):
        ...
        self.send_packet(
            ...
            self.buff_type.pack("dddff?", 8, 200, 8, 0, 90, 0b00000),
            self.buff_type.pack_varint(0),
        )
        ...
        logger.info(f"\033[033m{self.display_name} has joined.\033[0m")
    ...
    def send_empty_perimiter(self, size):
        for x in range(-size, size + 1):
            for z in range(-size, size + 1):
                if x == -size or x == size or z == -size or z == size:
                    builtins.queue.append(
                        [x, z, True, builtins.emptyHeight, [None] * 16, [1] * 256, []]
                    )
    ...
    def player_left(self):
        ...
        logger.info(f"{self.display_name} has left.")
    ...
    def read_and_send_chunk(self, x, z):
        ...
        builtins.queue.append(
            [px, pz, True, heightmap, sections, biomes, block_entities]
        )

    def update_chunks(self):
        if (
            math.floor(self.player.x / 16) == self.chunk.x
            and math.floor(self.player.z / 16) == self.chunk.z
        ):
            return

        self.read_and_send_chunk(
            math.floor(self.player.x / 16), math.floor(self.player.z / 16)
        )
    ...
    def packet_chat_message(self, buff):
        ...
        if p_text == "/list":
            ...
            logger.info(f"<{self.display_name}> {p_text}")
